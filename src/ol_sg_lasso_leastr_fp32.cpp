...
OLSGLassoLeastRvFP32::OLSGLassoLeastRvFP32(const arma::fmat& features,
...
void OLSGLassoLeastRvFP32::writeModelToXMLStream(std::ofstream& XMLFile)
{
  int i_level = 0;
...
void OLSGLassoLeastRvFP32::writeSparseMappedWeightsToStream(std::ofstream& MappedWeightsFile, std::ifstream& FeatureMap)
{
  std::string line;
...
arma::frowvec& OLSGLassoLeastRvFP32::Train(const arma::fmat& A,
                               const arma::frowvec& responses,
                               const arma::mat& weights,
                               const arma::rowvec& field,
                               std::map<std::string, std::string> slep_opts,
                               const bool intercept)
{
  ...
  std::vector<float> s(F), v(F), As(m), ATAs(F), g(F), Av(m);
  ...
  for (iterStep = 0; iterStep < opts_maxIter; iterStep++)
  {
    ...
    while (true)
    {
      ...
      r_sum = (double)dotf(v.data(), v.data(), F);
...
const arma::fcolvec OLSGLassoLeastRvFP32::altra(const arma::fcolvec& v_in,
                            const int n,
...
const double OLSGLassoLeastRvFP32::treeNorm(const arma::frowvec& x_in,
                            const int n,
                            const arma::mat& ind_mat,
                            const int nodes) const
{
...
const double OLSGLassoLeastRvFP32::computeLambda2Max(const arma::frowvec& x_in,
                            const int n,
                            const arma::mat& ind_mat,
                            const int nodes) const
{
...
void OLSGLassoLeastRvFP32::altra_inplace(float* x, const float* v, int n,
...
double OLSGLassoLeastRvFP32::treeNorm_flat(const float* x, int n,
                                  const double* ind, int nodes) const
{
    double tree_norm = 0;
    int i, j;
...
double OLSGLassoLeastRvFP32::computeLambda2Max_flat(const float* x, int n,
                                           const double* ind, int nodes) const
{
    int i, j;
    double twoNorm;
