...
OLSGLassoLogisticRFP32::OLSGLassoLogisticRFP32(const arma::fmat& features,
...
void OLSGLassoLogisticRFP32::writeModelToXMLStream(std::ofstream& XMLFile)
{
  int i_level = 0;
...
void OLSGLassoLogisticRFP32::writeSparseMappedWeightsToStream(std::ofstream& MappedWeightsFile, std::ifstream& FeatureMap)
{
  /*
  int i_level = 0;
  //std::string XMLString = "";
...
arma::frowvec& OLSGLassoLogisticRFP32::Train(const arma::fmat& features,
                               const arma::frowvec& responses,
                               const arma::mat& weights,
                               std::map<std::string, std::string> slep_opts,
                               const arma::rowvec& field,
                               const bool intercept)
{
  ...
  for (size_t i = 0; i < m; i++) {
    if (y[i] == 1.0f) p_flag.push_back(i);
    else not_p_flag.push_back(i);
  }

  // Sample weights
  std::vector<float> sample_weights(m);
  double m1, m2;
  if (opts_sWeight.size() == 2)
  {
    ...
    std::fill(sample_weights.begin(), sample_weights.end(), 1.0f/m);
  } else {
    std::fill(sample_weights.begin(), sample_weights.end(), 1.0f/m);
  }
  ...
  std::vector<float> s(n), v(n), b(m);
  std::vector<float> aa(m), bb(m), prob(m);
  std::vector<float> As_pre(m), g_pre(n);
  ...
  for (int iterStep = 0; iterStep < opts_maxIter; iterStep++)
  {
    ...
    // aa = -y % (As_pre + sc)
    for (size_t i = 0; i < m; i++) aa[i] = -y[i] * (As_pre[i] + (float)sc);
    ...
    // b = -weighty % (1 - prob)
    for (size_t i = 0; i < m; i++) b[i] = -weighty[i] * (1.0f - prob[i]);
	...
	if (iterStep == 0)
	{
	  if (estimate_l2)
	  {
		  ...
		  {
			overlapping(x.data(), gap, penalty2, v.data(), n, groupNum, (float)(lambda1/L), (float)(l2_high/L), opts_ind.memptr(), opts_field.memptr(), Y.data(), opts_maxIter2, opts_flag2, opts_tol2);
			float max_abs_x = 0;
		  ...
		  for(int i = 1; i <= 100; i = i + 1)
		  {
			...
			overlapping(x.data(), gap, penalty2, v.data(), n, groupNum, (float)(lambda1/L), (float)(l2_target/L), opts_ind.memptr(), opts_field.memptr(), Y.data(), opts_maxIter2, opts_flag2, opts_tol2);
			float max_abs_x = 0;
			for (size_t j = 0; j < (size_t)n; j++) { float ax = std::abs(x[j]); if (ax > max_abs_x) max_abs_x = ax; }
			if (max_abs_x == 0)
			{
    ...
    while (true)
    {
      ...
      overlapping(x.data(), gap, penalty2, v.data(), n, groupNum, (float)(lambda1/L), (float)(lambda2/L), opts_ind.memptr(), opts_field.memptr(), Y.data(), opts_maxIter2, opts_flag2, opts_tol2);
...
double OLSGLassoLogisticRFP32::computeLambda2Max_flat(const float* x, int n,
