                               ...
                               const bool intercept)
{
  ...
  if (opts_init != 2){
	  std::memcpy(x.data(), ATy.data(), n * sizeof(double));
  ...
  std::vector<double> s(n), v(n), Av(m);
  std::vector<double> As_pre(m), ATAs_pre(n), g_pre(n);
  ...
  for (int iterStep = 0; iterStep < opts_maxIter; iterStep++)
  {
    ...
    // funVal = 0.5*||Ax-y||^2 + lambda1*||x||_1 + lambda2*penalty2[0]
    double l1_norm = 0;
...
double OLSGLassoLeastRFP64::computeLambda2Max_flat(const double* x, int n,
