...
//std::cout << "A_rows:" << A.n_rows << " A_cols:" << A.n_cols << " b_elems:" << b.n_elem << std::endl;
    arma::fmat ATAs = A.t() * As;
    arma::fmat g = ATAs - ATy;
    xp = x;   Axp = Ax;
	if (iterStep == 0)
	{
	  if (estimate_l2)
...
/*
void OLSGLassoLeastRFP32::Predict(const arma::mat& points,
...
double OLSGLassoLeastRFP32::ComputeError(const arma::fmat& features,
                                      const arma::frowvec& responses) const
{
...
const arma::fcolvec OLSGLassoLeastRFP32::altra(const arma::fcolvec& v_in,
                            const int n,
...
const double OLSGLassoLeastRFP32::treeNorm(const arma::frowvec& x_in,
                            const int n,
                            const arma::mat& ind_mat,
                            const int nodes) const
{
...
const double OLSGLassoLeastRFP32::computeLambda2Max(const arma::frowvec& x_in,
                            const int n,
                            const arma::mat& ind_mat,
                            const int nodes) const
{
