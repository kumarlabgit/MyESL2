...
inline void identifySomeZeroEntries(float * u, int * zeroGroupFlag, int *entrySignFlag,
        int *pp, int *gg,
        float *v, float lambda1, float lambda2,
        int p, int g, float * w, float *G){
...
inline void xFromY(float *x, float *y,
        float *u, float *Y,
...
inline void YFromx(float *Y,
        float *xnew, float *Ynew,
        float lambda2, int g, int *zeroGroupFlag,
        float *G, float *w){
...
inline void dualityGap(float *gap, float *penalty2,
        float *x, float *Y, int g, int *zeroGroupFlag,
        float *G, float *w, float lambda2){

    int i,j;
    float temp, twoNorm, innerProduct;
...
inline void overlapping_gd(float *x, float *gap, float *penalty2,
        float *v, int p, int g, float lambda1, float lambda2,
        float *w, float *G, float *Y, int maxIter, int flag, float tol){

    int YSize=(int) w[3*(g-1) +1]+1;
    float *u=(float *)malloc(sizeof(float)*p);
    float *y=(float *)malloc(sizeof(float)*p);

    float *xnew=(float *)malloc(sizeof(float)*p);
    float *Ynew=(float *)malloc(sizeof(float)* YSize );

    int *zeroGroupFlag=(int *)malloc(sizeof(int)*g);
    int *entrySignFlag=(int *)malloc(sizeof(int)*p);
    int pp, gg;
    int i, j, iterStep;
    float twoNorm,temp, L=1, leftValue, rightValue, gapR, penalty2R;
    ...
    for(iterStep=0;iterStep<maxIter;iterStep++){
         ...
         *
         * void dualityGap(float *gap, float *penalty2,
         *               float *x, float *Y, int g, int *zeroGroupFlag,
         *               float *G, float *w, float lambda2)
         *
         */
        ...
        if ( (flag==0) || (flag==1 && iterStep < nextRestartStep )){
            ...
            memcpy(x, xnew, sizeof(float) * p);
            memcpy(Y, Ynew, sizeof(float) * YSize);
 ...
 *
 *  gradientDescentStep(float *xnew, float *Ynew,
        float *LL, float *u,
        float *x, float *Y, int p, int g, int * zeroGroupFlag,
        float *G, float *w)
 *
 */


inline void gradientDescentStep(float *xnew, float *Ynew,
        float *LL, float *u, float *y, int *entrySignFlag, float lambda2,
        float *x, float *Y, int p, int g, int * zeroGroupFlag,
        float *G, float *w){

    float twoNorm, temp, L=*LL, leftValue, rightValue;
    int i,j;
...
inline void overlapping_agd(float *x, float *gap, float *penalty2,
        float *v, int p, int g, float lambda1, float lambda2,
        float *w, float *G, float *Y, int maxIter, int flag, float tol){
    ...
    float *xS=(float *)malloc(sizeof(float)*p);
    float *YS=(float *)malloc(sizeof(float)* YSize );

    /*float *xp=(float *)malloc(sizeof(float)*p);*/
    float *Yp=(float *)malloc(sizeof(float)* YSize );
    ...
    float alpha, alphap=0.5, beta, gamma;
    /*
     * call the function to identify some zero entries
     *
     * entrySignFlag[i]=0 denotes that the corresponding entry is definitely zero
    ...
    /*memcpy(xp, x, sizeof(float) * p);*/
    memcpy(Yp, Y, sizeof(float) * YSize);
            ...
            if (*gap< gapR){
                ...
                *penalty2=penalty2R;
                ...
                if (*gap <=tol){
                    ...
                    memcpy(x, xS, sizeof(float) * p);
                    memcpy(Y, YS, sizeof(float) * YSize);
                    ...
                    /*memcpy(xp, xS, sizeof(float) * p);*/
                    memcpy(Yp, YS, sizeof(float) * YSize);
...
inline void overlapping(float *x, float *gap, float *penalty2,
        float *v, int p, int g, float lambda1, float lambda2,
        float *w, float *G, float *Y, int maxIter, int flag, float tol){
    switch(flag){
        case 0:
